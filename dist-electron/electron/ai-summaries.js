...
exports.saveAiSummary = saveAiSummary;
exports.listAiSummaries = listAiSummaries;
exports.getAiSummary = getAiSummary;
exports.deleteAiSummary = deleteAiSummary;
...
function saveAiSummary(authDb, payload) {
    ...
    const id = payload.id?.trim() || (0, node_crypto_1.randomUUID)();
    ...
    const existing = authDb
       ...
       LIMIT 1`)
        .get(id);
    authDb
        .prepare(`INSERT INTO ai_summaries (
         id,
         book_id,
         book_title,
         author,
       ...
       ON CONFLICT(id) DO UPDATE SET
        ...
        .run(id, bookId, bookTitle, author, language, summary, JSON.stringify(keyIdeas), JSON.stringify(studyNotes), JSON.stringify(flashcards), existing?.created_at ?? now, now);
    const row = authDb
        .prepare(`SELECT id, book_id, book_title, author, language, summary, key_ideas_json, study_notes_json, flashcards_json, created_at, updated_at
       FROM ai_summaries
       WHERE id = ?
       LIMIT 1`)
        .get(id);
...
function listAiSummaries(authDb) {
    const rows = authDb
        .prepare(`SELECT id, book_id, book_title, author, language, summary, key_ideas_json, study_notes_json, flashcards_json, created_at, updated_at
       FROM ai_summaries
...
function getAiSummary(authDb, payload) {
...
function deleteAiSummary(authDb, payload) {
    const id = payload.id?.trim();
    if (!id) {
        return { ok: false, error: 'AI summary not found.' };
    }
    const result = authDb
        .prepare(`DELETE FROM ai_summaries
       WHERE id = ?`)
        .run(id);
    if (result.changes === 0) {
        return { ok: false, error: 'AI summary not found.' };
