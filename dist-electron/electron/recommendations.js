...
exports.getRecommendationsForBook = getRecommendationsForBook;
exports.rankDiscoverResultsForUser = rankDiscoverResultsForUser;
...
function getSubjectTokens(book) {
    return Array.from(new Set((book.subjects ?? [])
        .flatMap((subject) => tokenize(subject))
        .filter(Boolean)));
}
function getAuthorKey(book) {
    return normalizeValue(book.author);
}
function getReadingSignals(db, userId) {
    return db
        .prepare(`SELECT
         b.id AS book_id,
...
function buildProfile(books, signals) {
...
function toSummary(books, profile) {
...
function getTitleOverlap(left, right) {
...
function rankHomeRecommendations(books, profile) {
...
function rankSimilarBooks(books, targetBook, profile) {
...
function rankMoreByAuthor(books, targetBook, profile) {
    const targetAuthor = getAuthorKey(targetBook);
    if (!targetAuthor) {
        return [];
    }
...
function getHomeRecommendations(db, userId) {
    const booksResult = (0, books_1.listBooks)(db, userId);
    if (!booksResult.ok) {
        return booksResult;
    }
...
function getRecommendationsForBook(db, userId, payload) {
...
function scoreDiscoverResultAgainstQuery(query, result) {
...
function getSourcePriorityBoost(source) {
    return source === 'standardebooks' ? 8 : 4;
}
function rankDiscoverResultsForUser(db, userId, query, results) {
    const booksResult = (0, books_1.listBooks)(db, userId);
...
function scoreDiscoverResultForProfile(result, profile) {
