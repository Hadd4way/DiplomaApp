...
import {
  isSameFb2PointLocation,
  parseFb2Location,
  serializeFb2PointLocation,
  serializeFb2RangeLocation
...
function getBlockElement(container: ParentNode | null, blockId: string | null | undefined): HTMLElement | null {
  ...
  const node = container.querySelector(`[data-fb2-block-id="${escaped}"]`);
...
function getClosestBlockElement(node: Node | null): HTMLElement | null {
  ...
  if (node instanceof HTMLElement) {
    return node.closest('[data-fb2-block-id]') as HTMLElement | null;
  }
  return node.parentElement?.closest('[data-fb2-block-id]') as HTMLElement | null;
}
...
function serializeSelectionRange(article: HTMLElement, selection: Selection): { location: string; text: string | null } | null {
  ...
  const startBlockId = startBlock?.dataset.fb2BlockId ?? null;
  const endBlockId = endBlock?.dataset.fb2BlockId ?? null;
  if (!chapterId || !startBlock || !endBlock || !startBlockId || !endBlockId) {
    return null;
  }

  return {
    location: serializeFb2RangeLocation({
      chapterId,
      startBlockId,
...
function buildRangeFromLocation(article: HTMLElement, location: string): Range | null {
  const parsed = parseFb2Location(location);
...
export function Fb2ReaderScreen({ title, bookId, initialCfi = null, onInitialCfiApplied, loading, onBack }: Props) {
  ...
  const bookmarksState = useFlowBookmarks({
    ...
    matchLocation: isSameFb2PointLocation
  });
  ...
  const navigateToSearchIndex = React.useCallback(
    (index: number) => {
      ...
      if (target) {
        const chapterId = documentData?.chapters[result.chapterIndex]?.id ?? `fb2-chapter-${result.chapterIndex}`;
        scrollToLocation(serializeFb2PointLocation({ chapterId, blockId: result.blockId }));
        flashSearchTarget(target);
      }
    },
    [documentData?.chapters, flashSearchTarget, scrollToLocation, searchResults]
  );
  ...
  React.useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const blockElements = Array.from(article.querySelectorAll('[data-fb2-block-id]')).filter(
        (node): node is HTMLElement => node instanceof HTMLElement
      );
      let activeBlockId: string | null = null;
      for (const block of blockElements) {
        if (block.offsetTop <= scrollMarker) {
          activeBlockId = block.dataset.fb2BlockId ?? null;
        } else {
          break;
        }
      }

      const chapterId = documentData.chapters[nextChapterIndex]?.id ?? `fb2-chapter-${nextChapterIndex}`;
      setCurrentLocation(serializeFb2PointLocation({ chapterId, blockId: activeBlockId }));
  ...
  const goPrevChapter = () => {
    ...
    const chapterId = documentData?.chapters[nextChapter]?.id ?? `fb2-chapter-${nextChapter}`;
    scrollToLocation(serializeFb2PointLocation({ chapterId }));
  };
                ...
                {documentData.chapters.map((chapter, index) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        registerActivity();
                        scrollToLocation(serializeFb2PointLocation({ chapterId: chapter.id }));
                      }}
                    >
                      {chapter.title}
                    </button>
        ...
        rightPanel={
          <>
            <SearchPanel
              ...
              placeholder="Search in this FB2..."
            ...
            >
              <div className="space-y-2">
                ...
                {bookmarksState.bookmarks.map((bookmark: EpubBookmark) => {
                  const active = isSameFb2PointLocation(currentLocation, bookmark.cfi);
                  return (
                    <div
                      key={bookmark.id}
        ...
        footer={
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="inline-flex items-center gap-2 text-xs" style={{ color: palette.mutedText }}>
              ...
              {settingsLoading ? 'Loading settings...' : ready ? 'Ready' : 'Loading FB2...'}
            </div>
            <Button
              type="button"
              variant="outline"
