...
export type OpenLibraryMetadataMatch = {
  title: string;
...
const workDetailsCache = new Map<string, Promise<OpenLibraryWorkResponse | null>>();
const editionDetailsCache = new Map<string, Promise<OpenLibraryEditionResponse | null>>();
...
function toAbsoluteOpenLibraryUrl(pathname: string) {
  return new URL(pathname, OPEN_LIBRARY_BASE_URL).toString();
}

function extractOlid(value: string | null | undefined) {
  if (!value) {
    return null;
  }
...
function extractWorkId(value: string | null | undefined) {
...
function extractEditionId(value: string | null | undefined) {
  const olid = extractOlid(value);
  if (olid?.endsWith('M')) {
    return olid;
  }
...
function parseDescription(value: string | { value?: string } | null | undefined) {
...
function dedupeSubjects(subjects: Array<string | null | undefined>) {
...
function buildBookCoverUrl(key: 'olid' | 'isbn' | 'id', value: string | number | null | undefined, size: 'S' | 'M' | 'L' = 'L') {
...
function scoreMatch(doc: OpenLibrarySearchDoc, title: string, author?: string | null) {
  ...
  if (normalizedDocTitle === normalizedRequestedTitle) {
    ...
    normalizedDocTitle.startsWith(normalizedRequestedTitle) ||
    normalizedRequestedTitle.startsWith(normalizedDocTitle)
  ) {
    score += 70;
  } else if (
    normalizedDocTitle.includes(normalizedRequestedTitle) ||
  ...
  if (normalizedRequestedAuthor) {
    if (normalizedDocAuthors.some((item) => item === normalizedRequestedAuthor)) {
      ...
      normalizedDocAuthors.some(
        (item) => item.includes(normalizedRequestedAuthor) || normalizedRequestedAuthor.includes(item)
      )
    ) {
      score += 45;
    }
...
function fetchWorkDetails(workId: string | null | undefined) {
  ...
  const request = fetchOpenLibraryJson<OpenLibraryWorkResponse>(
    toAbsoluteOpenLibraryUrl(`/works/${normalizedWorkId}.json`)
  ).catch(() => null);
...
function fetchEditionDetails(editionId: string | null | undefined) {
  ...
  const request = fetchOpenLibraryJson<OpenLibraryEditionResponse>(
    toAbsoluteOpenLibraryUrl(`/books/${normalizedEditionId}.json`)
  ).catch(() => null);
...
function toMatchFromDoc(doc: OpenLibrarySearchDoc): OpenLibraryMetadataMatch {
  ...
  const coverUrl =
    buildBookCoverUrl('olid', editionId) ??
...
export async function findBestMatch(title: string, author?: string | null): Promise<OpenLibraryMetadataMatch | null> {
  ...
  url.searchParams.set(
    'fields',
    ['key', 'title', 'subtitle', 'author_name', 'cover_i', 'cover_edition_key', 'edition_key', 'isbn', 'first_publish_year', 'subject'].join(',')
  );
  ...
  const payload = await fetchOpenLibraryJson<OpenLibrarySearchResponse>(url.toString()).catch(() => null);
...
export function getCover(lookup: OpenLibraryLookup): string | null {
  return (
    buildBookCoverUrl('olid', extractEditionId(lookup.editionId)) ??
    buildBookCoverUrl('isbn', lookup.isbn?.trim() || null) ??
    buildBookCoverUrl('id', lookup.coverId ?? null)
  );
}

export async function getDescription(lookup: OpenLibraryLookup): Promise<string | null> {
...
export async function getSubjects(lookup: OpenLibraryLookup): Promise<string[]> {
  const [workDetails, editionDetails] = await Promise.all([
    fetchWorkDetails(lookup.workId),
    fetchEditionDetails(lookup.editionId)
  ]);
...
export async function getPublishYear(lookup: OpenLibraryLookup): Promise<number | null> {
  const [workDetails, editionDetails] = await Promise.all([
    fetchWorkDetails(lookup.workId),
    fetchEditionDetails(lookup.editionId)
  ]);
...
export const openLibraryMetadataProvider = {
  findBestMatch,
  getCover,
  getDescription,
  getSubjects,
