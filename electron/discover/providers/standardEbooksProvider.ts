...
type StandardEbooksSearchEntry = {
  path: string;
  title: string;
  author: string | null;
...
function ensureDownloadSourceUrl(value: string | null) {
  if (!value) {
    return null;
  }
...
function createEpubFormat(url: string): DiscoverBookFormatDescriptor {
  return {
    kind: 'epub',
...
function parseSearchEntries(html: string): StandardEbooksSearchEntry[] {
  ...
  const blocks = Array.from(
    sectionMatch[1].matchAll(
      /<li\b(?=[^>]*typeof="schema:Book")[\s\S]*?<\/li>\s*(?=(?:<li\b(?=[^>]*typeof="schema:Book"))|$)/gi
    ),
    (match) => match[0]
  );
  ...
  return blocks
    .map((block) => {
      ...
      return {
        ...
        author,
        coverUrl
      } satisfies StandardEbooksSearchEntry;
    })
    .filter((entry): entry is StandardEbooksSearchEntry => Boolean(entry));
}

function parseMetaContent(html: string, key: string) {
  return (
...
function parseCompatibleEpubUrl(html: string) {
  const compatibleAnchorMatch = html.match(
    /<a[^>]+href="([^"]+\.epub[^"]*)"[^>]*>\s*(?:<[^>]+>\s*)*Compatible epub\b/i
  );
  if (compatibleAnchorMatch?.[1]) {
    return ensureDownloadSourceUrl(toAbsoluteUrl(compatibleAnchorMatch[1]));
  }

  const nearbyHeadingMatch = html.match(
    /Download for ereaders[\s\S]*?<a[^>]+href="([^"]+\.epub[^"]*)"[^>]*>[\s\S]*?Compatible epub/i
  );
  if (nearbyHeadingMatch?.[1]) {
    return ensureDownloadSourceUrl(toAbsoluteUrl(nearbyHeadingMatch[1]));
  }

  const genericEpubLinks = Array.from(html.matchAll(/href="([^"]+\.epub[^"]*)"/gi), (match) => match[1]).map(
    (value) => value.trim()
  );
  const compatibleCandidate = genericEpubLinks.find(
    (value) => !/advanced|kepub/i.test(value) && !/thumbnail/i.test(value)
  );

  return ensureDownloadSourceUrl(toAbsoluteUrl(compatibleCandidate ?? null));
}

function getDetailDescription(html: string) {
...
async function fetchDetail(entry: StandardEbooksSearchEntry): Promise<DiscoverBookResult | null> {
...
export const standardEbooksProvider = {
  async search(query: string, options: Omit<DiscoverSearchRequest, 'query'> = {}): Promise<DiscoverBookResult[]> {
    ...
    return results.filter((result): result is DiscoverBookResult => Boolean(result));
  }
};
